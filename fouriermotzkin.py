...
print 'Enter 1 if all the equations are inequalities'
print 'Enter 2 if some equations are equalities'
...
if (choice == 1):
	...
	print 'Convert all equations to inequalities by breaking them in <= and >= form'
	print 'Multiply all equations of form Ax>=b by -1'
	print 'Enter dimensions of matrix A:'
	...
	a = [[0.0 for i in range (0,int(n))] for j in range (0,int(m))]
	print 'Enter elements of A (one row at a time):'
	for i in range (0, int(m)):
		a[i] = [float(x) for x in sys.stdin.readline().split()]

	b = [[0.0 for i in range (1)] for j in range (int(m))]
	print 'Enter elements of b (b1 b2 b3 . . . bn):'
	b = [float(x) for x in sys.stdin.readline().split()]
