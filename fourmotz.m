...
%     ones(nneg,1)*temp(1,2:n+1)+temp(npos+1:npos+nneg,2:n+1)
    ones(nneg,1)
    disp('Appropriate A is ')
    A
    disp('A after transform : ')
	for i=1:npos,
...
disp ('in else if')
A
%     b = A(:,n+1);			% extract vector b
	A = A(:,1:n);			% reduce to the relevant A
end
